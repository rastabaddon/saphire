...
{
	if(core&&core->getMemoryManager()) return core->getMemoryManager()->deallocate((Saphire::Core::Types::pointer)pointerToDelete);
   free(pointerToDelete);
}

void operator delete[ ] (void* arrayToDelete)
{
	if(core&&core->getMemoryManager())  return core->getMemoryManager()->deallocate((Saphire::Core::Types::pointer)arrayToDelete);
	free(arrayToDelete);
}
